...
/// App-owned NFC session provider for the PoPP check-in flow.
///
/// ScoopPoppSDK and ScoopCardlink each bundle NFC separately, so their
/// `NfcTransceiverProvider` / `IosNfcTransceiver` are distinct Swift types — this
/// PoPP-scoped provider mirrors `DemoCardlinkNfcProvider` but imports ScoopPoppSDK.
///
/// The SDK's turnkey `IosNfcTransceiverProvider` doesn't deliver `didDetect` on
/// current builds; this owns the `NFCTagReaderSession` in Swift (the proven
/// NfcDemo pattern) so the tag is delivered and connected.
///
/// Pass an instance as `PoppFlowConfig(nfcProvider:)`.
final class DemoPoppNfcProvider: NSObject, NfcTransceiverProvider,
